...
app.post("/usuarios", async (req, res) => {
  try {
    const { nome, usuario, senha, nivel, celula } = req.body;
    ...
    await pool.query(
      ...
      [
        ...
        montarNomeCelulaExibicao(celula)
      ]
    );
...
async function iniciarServidor() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`+Células Backend V16 rodando na porta ${PORT}`);
    });
  } catch (erro) {
